...
public class textBoxShadow : MonoBehaviour
{
    /*
     * Hello Team :)
     * 
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.collider.tag == "Player")
        {
            ...
            //Add extra code here
            Player.GetComponent<PlayerHealth>().invulnerableBuff = true;
            Popup.GetComponent<AbilityPopup>().abilityIndex = 4;
            Popup.GetComponent<AbilityPopup>().popup();
            // Keep this as last line
            otherOption = GameObject.Find("Crescendo(Clone)");
