...
public class textBoxFear : MonoBehaviour
{
    /*
     * Hello Team :)
     * 
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.collider.tag == "Player")
        {
            ...
            //Add extra code here
            Player.GetComponent<BulletReversal>().canSlow = true;
            Popup.GetComponent<AbilityPopup>().abilityIndex = 2;
            Popup.GetComponent<AbilityPopup>().popup();
            // Keep this as last line
            otherOption = GameObject.Find("Clear(Clone)");
            Destroy(otherOption);
            Destroy(gameObject);
        }
    }
