...
public class textBoxClear : MonoBehaviour
{
    /*
     * Hello Team :)
     * 
     ...
     *
     * Other than that, customize it however you need to change aspects of the game like palyer abilities and dragon stats
     */
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.collider.tag == "Player")
        {
            ...
            //Add extra code here
            Player.GetComponent<PlayerMovment>().canClear = true;
            Popup.GetComponent<AbilityPopup>().abilityIndex = 1;
            Popup.GetComponent<AbilityPopup>().popup();
            // Keep this as last line
            otherOption = GameObject.Find("Fear(Clone)");
            Destroy(otherOption);
            Destroy(gameObject);
        }
    }
